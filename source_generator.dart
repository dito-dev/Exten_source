...
List<Source> _searchJsSources(Directory dir) {
  ...
  for (FileSystemEntity entity in entities) {
    if (entity is Directory) {
      if (entity.path.contains('not_working') || entity.path.contains('deprecated') || entity.path.contains('.git')) {
        continue;
      }
      sourceList.addAll(_searchJsSources(entity));
    } else if (entity is File && entity.path.endsWith('.js')) {
      final regex = RegExp(
        r'(?:const|var|let)\s+mangayomiSources\s*=\s*(\[.*?\]);',
        dotAll: true,
      );
      ...
      if (match != null) {
        for (var sourceJson in jsonDecode(match.group(1)!) as List) {
          ...
          Source source = Source.fromJson(sourceJson)
            ...
            ..sourceCodeUrl =
                "https://raw.githubusercontent.com/dito-dev/Exten_source/main/${sourceJson["pkgPath"] ?? sourceJson["pkgName"]}";
